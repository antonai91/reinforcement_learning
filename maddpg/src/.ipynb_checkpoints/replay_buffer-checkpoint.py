...
class ReplayBuffer():
    ...
    def save(self, folder_path):
        ...
        for index in range(self.n_agents):
            np.save(folder_path + '/actor_states_{}.npy'.format(index), self.list_actors_states[index])
            np.save(folder_path + '/actor_next_states_{}.npy'.format(index), self.list_actors_next_states[index])
            np.save(folder_path + '/actor_actions_{}.npy'.format(index), self.list_actors_actions[index])
    ...
    def load(self, folder_path):
        ...
        self.list_actors_states = [np.load(folder_path + '/actor_states{}.npy'.format(index)) for index in range(self.n_agents)]
        self.list_actors_next_states = [np.load(folder_path + '/actor_next_states_{}.npy'.format(index)) for index in range(self.n_agents)]
        self.list_actors_actions = [np.load(folder_path + '/actor_actions_{}.npy'.format(index)) for index in range(self.n_agents)]
