...
class A2CAgent:
    def __init__(self, model, save_path=None, load_path=None, lr=1e-4, gamma=0.99, value_c=0.5, entropy_c=1e-4, clip_ratio=0.2):
    ...
    def train(self, wrapper, batch_sz=64, updates=1000000, input_shape=(4, )):
        # Storage helpers for a single batch of data.
        actions = np.empty((batch_sz,), dtype=np.int32)
        rewards, dones, values = np.empty((3, batch_sz))
        observations = np.empty((batch_sz,) + input_shape)
        old_logits = np.empty((batch_sz, wrapper.action_space.n), dtype=np.float32)
        ...
        for update in range(updates):
            ...
            for step in range(batch_sz):
                ...
                next_obs, rewards[step], dones[step], _ = wrapper.step(actions[step])
                #next_obs = wrapper.state
                ep_rewards[-1] += rewards[step]
                if dones[step]:
                    ep_rewards.append(0.0)
                    next_obs = wrapper.reset()
                    print({'Game number': len(ep_rewards) - 1, '# Update': update, '% Update': round(update / updates, 2), 
            ...
            #returns, advs = self._new_returns_advantages(rewards, dones, values, next_value)

            # Performs a full training step on the collected batch.
            # Note: no need to mess around with gradients, Keras API handles it.
            with tf.GradientTape() as tape:
                logits, v = self.model(observations, training=True)
                #logit_loss = self._logits_loss(actions, advs, logits)
                logit_loss = self._new_logits_loss(old_logits, logits, actions, advs, wrapper.action_space.n)
    ...
    def _logits_loss(self, actions, advantages, logits):
    ...
    def _new_logits_loss(self, old_logits, logits, actions, advs, n_actions):
